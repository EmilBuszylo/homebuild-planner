...
- Full test strategy and cookbook recipes → `context/foundation/test-plan.md` (§6). Local gates: `pnpm test`, `pnpm lint`. GitHub Actions CI (`.github/workflows/ci.yml`) runs the same plus `pnpm build:ci` on pull requests — see test-plan §5.
